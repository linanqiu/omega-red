...
Requester.prototype.all = function (count, cb) {
	var limit = this.url.query.limit;
	this.url.query.limit = (limit) ? limit : 100; //default max limit, 100

	cb(this.ee);

	// console.log("All with count %d limit %d", count, this.url.query.limit);

	this.collector(count, this.url.query.limit);
};
...
Requester.prototype.collector = function (count, limit) {
	var that = this,
		...
		if (!error && res.statusCode == 200) {
			...
			if (parsedBody.data.after) {
				...
				return (nextAfter !== prevAfter) ? //a check to see if we are done
					that.collector(count, limit) : that.ee.emit('end');
			}
...
var queries = [
	'sort', 'from', 'limit', 'after', 'before', 'count'
];
...
var filters = [
	//user
	'overview', 'comments', 'submitted', 'liked', 'disliked', 'hidden', 'saved', 'about'
	//r
